...
/* Initialization of a long array of Vectors and Matrices*/
...
#pragma omp parallel default(shared) private(i, j) 
  { 
    for(i=0; i<reps; i++)
    {
        ...
        for(j=0; j<in; j++)
        { 
            single_MVM_macro((chi+j), (up+j), (psi+j));
        }
    }
  }
  gettimeofday(&end, 0);
 ...
 */
  ...
  flop = in * 3*((3*4+3*2+4)); /* (36 muls, 30 adds in single MVM) */
  byte = in * (3 * sizeof(suNf_vector) + sizeof(suNf));/* 6 as in order to update the content of a memory address you must anyway first read the content of it and then write so it is 4 sunf_vectors in read and 2 sunf_vectors in write */
  mb = (float) (byte)/1.e6;
...
{ 
    for(i=0; i<reps; i++)
    {
        ...
        for(j=0; j<in; j++)
        { 
             _suNf_theta_T_multiply((*(chi2+j)), *((up+j)), *((psi+j)));
        }
    }
}
  gettimeofday(&end, 0);
...
for (i = 0; i < in; i++)
{
    ...
    res9 =  _complex_re((chi2+i)->c[j]); 
    res10 = _complex_im((chi2+i)->c[j]);  
